...
async def main() -> None:
    async with Actor:
        ...
        Actor.log.info(
            "Starting Clinical Trials Scraper | mode=%s | max_results=%s",
            config.mode.value,
            max_results,
        )
        ...
        await Actor.set_status_message("Connecting to ClinicalTrials.gov...")

        async with httpx.AsyncClient() as client:
            rate_limiter = RateLimiter(interval=config.request_interval_secs)
            scraper = ClinicalTrialsScraper(client, rate_limiter, config)
            ...
            try:
                async for item in scraper.scrape():
                    ...
                    if len(batch) >= batch_size:
                        ...
                        batch = []
                        await Actor.set_status_message(f"Scraped {count}/{max_results} studies")
        ...
        msg = f"Done. Scraped {count} studies."
        if state["failed"] > 0:
